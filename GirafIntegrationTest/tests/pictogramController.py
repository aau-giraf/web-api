...
class PictogramController(TestCase):
    ...
    def getAllPictograms(self, check):
        ...
        response = requests.get(Test.url + 'Pictogram?page=1&pageSize=10', headers = {"Authorization":"Bearer {0}".format(PictogramController.kurtToken)}).json()
        check.is_true(response['success'])
    ...
    def getPublicPictogram(self, check):
        ...
        response = requests.get(Test.url + 'Pictogram/2', headers = {"Authorization":"Bearer {0}".format(PictogramController.kurtToken)}).json()
        check.is_true(response['success'])
    ...
    def getPosted(self, check):
        "Get the freshly posted Fish"
        response = requests.get(Test.url + 'Pictogram/{0}'.format(PictogramController.fishID), headers = {"Authorization":"Bearer {0}".format(PictogramController.kurtToken)}).json()
    ...
    def getPublicPictogramImage(self, check):
        ...
        response = requests.get(Test.url + 'Pictogram/2/image', headers = {"Authorization":"Bearer {0}".format(PictogramController.kurtToken)}).json()
        check.is_true(response['success'])
        check.is_not_none(response['data'])
    ...
    def getPublicPictogramImageRaw(self, check):
        ...
        response = requests.get(Test.url + 'Pictogram/2/image/raw', headers = {"Authorization":"Bearer {0}".format(PictogramController.kurtToken)})
        check.equal(response.status_code, 200)

    @test(skip_if_failed=["loginAsKurt", "post"])
    def putImage(self, check):
        "Put image into Cursed Fish"
        response = requests.put(Test.url + 'Pictogram/{0}/image'.format(PictogramController.fishID), data=rawSampleImage, headers = {"Authorization":"Bearer {0}".format(PictogramController.kurtToken)}).json()
        check.is_true(response['success'])

    @test(skip_if_failed=["loginAsKurt", "putImage", "checkPut"])
    def deletePictogram(self, check):
        "Delete the Cursed Fish"
        response = requests.delete(Test.url + 'Pictogram/{0}'.format(PictogramController.fishID), headers = {"Authorization":"Bearer {0}".format(PictogramController.kurtToken)}).json()
        check.is_true(response['success'])
