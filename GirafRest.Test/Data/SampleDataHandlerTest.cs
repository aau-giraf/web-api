...
namespace GirafRest.Test
{
    public class SampleDataHandlerTest
    {
        ...
        public async void DeserializeDataTest()
        {
            ...
            Assert.NotNull(data);
            Assert.Equal(new string("Admin"), users[0].Name);
            Assert.Equal(new string("Guardian in dep 2"), users[1].Name);
            Assert.Equal(new string( "Citizen of dep 2"), users[2].Name);
            Assert.Equal(new string("Mock Department"), deps[0].Name);
            Assert.Equal(new string("Mock Department2"), deps[1].Name);
            Assert.Equal(new string("Picto 1"), pics[0].Title);
            Assert.Equal(new string("Public Picto2"), pics[1].Title);
            Assert.Equal(new string("No restrictions"), pics[2].Title);
            Assert.Equal(new string( "My awesome week"), weeks[0].Name);
            Assert.Equal(new string("My not so awesome week"), weeks[1].Name);
            Assert.Equal(new string("Template1"), weekTemplates[0].Name);
            Assert.Equal(new string("Template2"), weekTemplates[1].Name);
